...
public abstract class BaseAudioReactiveEffect : IEffect
{
   ...
   //private double _sample = 0; // Used to convert returned value to have '0' as minimum.
    ...
    private void Smooth(ref double[] buffer)
    {
        ...
        switch (AvgSmoothingMode)
        {
            ...
            case AvgSmoothingMode.All:
                ...
                SampleAvg = (SampleAvg * 15 + buffer.Rms()) / 16;
                break;
        }
    }
    ...
    private FftCalibration _fftCal = new(257);
    private void DoFFT(double[] buffer)
    {
        ...
        /*_fftCal.AddDataset(FftBins);
        var cal = _fftCal.ToAdjustmentData(50);
        Console.WriteLine(Math.Round(cal[16] * FftBins[16], 4) + " ---- " + _fftCal.ToCsv(50));
        Console.WriteLine("---------------------");*/
        
        for (var index = 0; index < FftBins.Length; index += FftBins.Length / 10)
        {
            //Console.WriteLine($"[{index}]\t{Math.Round(FftFreq[index])}Hz\t=\t{Math.Round(FftBins[index], 4)}");
        }
        //Console.WriteLine("-------------------");
